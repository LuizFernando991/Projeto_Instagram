...
export function CreatePostComponent({
    ...
    const transition = useTransition(isCreatePostOpen, {
    ...
    return (
        <>
            <Styled.CreatePostContainer onClick={() => setIsCreatePostOpen(false)}>
                <RiCloseLine className="close-button" />
            </Styled.CreatePostContainer>
            {transition((style, item) =>
                item ? (
                    <Styled.CreatePost style={style}>
                        <div className="create-post-header">
                            <h3>Create Post</h3>
                        </div>
                        <Styled.ChosePost>
                            <input type="file" accept="image/*" style={{ display: 'none' }} ref={storieInput} />
                            <svg
